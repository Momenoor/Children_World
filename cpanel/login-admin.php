	...
	$password=$_POST['password'];
{
	...
	$count = mysqli_num_rows($result);				
		if ($count == 0) 
			{
			echo "<script>alert('من فضلك تأكد من اسم المستخدم أو كلمة المرور!'); window.location='login-admin.php'</script>";
			} 
		else if ($count > 0)
			{
				session_start();
			...
			$result1 = mysqli_query($conn,"SELECT * FROM `admin`  WHERE username = '$username' and `password`='".md5($password)."'");
