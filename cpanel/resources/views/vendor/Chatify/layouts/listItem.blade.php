...
@if($get == 'saved')
    <table class="messenger-list-item m-li-divider" data-contact="{{ Auth::user()->id }}">
        <tr data-action="0">
            ...
            {{-- center side --}}
            <td>
                <p data-id="{{ Auth::user()->id }}" data-type="user">{{trans('backpack::app.Saved Messages')}} <span> {{trans('backpack::app.You')}} </span></p>
                <span>{{trans('backpack::app.Save messages secretly')}}</span>
            </td>
        </tr>
    </table>
@endif
...
<table class="messenger-list-item" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <span>
            ...
            !!}
            @else
            <span class="fas fa-file"></span> {{trans('backpack::app.Attachment')}}
            @endif
        </span>
