    ...
    <tr>
        ...
        @foreach($table->columns() as $column)
            <th
                ...
                class="@if($loop->first && $hasBulkActions) pl-6 @else px-6 @endif py-3 text-left rtl:text-right text-xs font-medium tracking-wide text-gray-500"
            >
                @if($column->sortable)
                ...
                <span class="flex flex-row items-center">
                    ...
                    @if($column->sortable)
                        <svg aria-hidden="true" class="w-3 h-3 mr-2 @if($column->sorted) text-green-500 @else text-gray-400 @endif" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                            @if(!$column->sorted)
