...
$_SESSION['teacher-id'] = 1;
$_SESSION['teacher'] = 'Momen';
...
{
	$result = mysqli_query($conn,"SELECT * FROM teacher  WHERE username = '$username' and pass='".md5($password)."'");
	$row = mysqli_fetch_array($result);
	$count = mysqli_num_rows($result);				
		if ($count == 0) 
			{
			echo "<script>alert('من فضلك تأكد من اسم المستخدم أو كلمة المرور!'); window.location='login-teacher.php'</script>";
			} 
		else if ($count > 0)
			{
				session_start();
			...
			$result1 = mysqli_query($conn,"SELECT * FROM teacher  WHERE username = '$username' and pass='".md5($password)."'");
